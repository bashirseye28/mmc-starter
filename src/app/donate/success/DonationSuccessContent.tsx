...
import {
  faArrowRight, faArrowLeft, faCheckCircle, faFilePdf,
  faEnvelope, faMoneyBillWave, faCalendarAlt, faUser,
      ...
      <div className="bg-white shadow-xl p-10 rounded-lg text-center max-w-lg border border-gray-200">
        ...
        >
          ...
          <div className="mt-3 space-y-3">
            <Info label="Donor" value={donationDetails.donorName} icon={faUser} />
            <Info label="Email" value={donationDetails.donorEmail} icon={faEnvelope} />
            <Info label="Amount" value={`£${donationDetails.amount}`} icon={faMoneyBillWave} />
            <Info label="Frequency" value={donationDetails.frequency} icon={faCalendarAlt} />
            <Info label="Method" value={donationDetails.method} icon={faMoneyBillWave} />
            <Info label="Date" value={donationDetails.date} icon={faCalendarAlt} />
          </div>
        </motion.div>
...
function Info({ label, value, icon }: { label: string; value: string; icon: any }) {
  return (
    <p className="text-lg font-semibold text-primary flex items-center gap-2">
      ...
      {label}: <span className="text-darkText">{value}</span>
    </p>
  );
}